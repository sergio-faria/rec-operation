...
def none_lists(m_dims: int) -> List[float]:
    """
...
def dict_none_lists(m_dim: int, designations: List[str]) -> Dict[str, List[float]]:
    """
...
def time_intervals(horizon: Union[int, float], delta_t: Union[int, float], func='int') -> int:
    ...
    :param delta_t: duration of the time intervals in which the horizon is divided; provided in hours
